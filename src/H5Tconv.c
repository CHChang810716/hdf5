        ...
        case H5T_CONV_CONV:
        {
            ...
            while(nelmts > 0) {
                ...
                for(elmtno = 0; elmtno < safe; elmtno++) {
                    ...
                    /* Read reference */
                    if((*(src->shared->u.atomic.u.r.read))(src->shared->u.atomic.u.r.f, s, conv_buf, buf_size) < 0)
                        HGOTO_ERROR(H5E_DATATYPE, H5E_READERROR, FAIL, "can't read reference data")

                    /* Write reference to destination location */
                    if((*(dst->shared->u.atomic.u.r.write))(dst->shared->u.atomic.u.r.f, d, conv_buf, b, buf_size, ref_type) < 0)
